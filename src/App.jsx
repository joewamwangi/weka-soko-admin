...
function Users({token,notify}){
  ...
  useEffect(()=>{
    req("/api/admin/users",{},token).then(data=>{
      // users endpoint returns array or {users:[]}
  ...
  return <>
    ...
    <div className="tw">{loading?<div style={{textAlign:"center",padding:40}}><Spin/></div>:
      <div className="ts"><table>
        <thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Joined</th><th>Violations</th><th>Unlocks Paid</th><th>Status</th><th>Actions</th></tr></thead>
      ...
      <div className="ts"><table>
        ...
        <tbody>{listings.length===0?<tr><td colSpan={7}><div className="empty">No listings found</div></td></tr>:listings.map(l=><tr key={l.id}>
          ...
          <td><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
            ...
            {l.status==="active"&&<button className="btn bs sm" onClick={()=>updStatus(l.id,"deleted")}>Suspend</button>}
...
function Reports({token,notify}){
  ...
  const load=useCallback(()=>{
    setLoading(true);
    req(`/api/admin/reports?status=${status}`,{},token)
  ...
  const resolve=async(id,action)=>{
    try{
      await req(`/api/admin/reports/${id}`,{method:"PATCH",body:JSON.stringify({action})},token);
      setReports(p=>p.filter(r=>r.id!==id));
      notify(`Report ${action}d.`,true);
    }catch(e){notify(e.message,false);}
  };

  const restore=async(listingId)=>{
    setRestoring(listingId);
    try{
      await req(`/api/admin/listings/${listingId}/restore`,{method:"POST"},token);
        ...
        {reports.map(r=><div key={r.id} className="card" style={{display:"flex",gap:14,alignItems:"flex-start"}}>
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:"var(--dim)"}}>
              Reported by {r.reporter_name} ({r.reporter_email}) · {new Date(r.created_at).toLocaleDateString()}
            </div>
          </div>
...
const SECTIONS=[{id:"overview",icon:"📊",label:"Overview"},{id:"users",icon:"👥",label:"Users"},{id:"listings",icon:"📦",label:"Listings"},{id:"reports",icon:"🚩",label:"Reports"},{id:"violations",icon:"🚨",label:"Violations"},{id:"escrow",icon:"🔐",label:"Escrow & Disputes"},{id:"payments",icon:"💳",label:"Payments"},{id:"vouchers",icon:"🎟️",label:"Vouchers"}];
